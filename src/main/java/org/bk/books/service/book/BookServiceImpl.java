...
public class BookServiceImpl implements BookService {
  ...
  public Book createBook(CreateBookCommand command) {
    ...
    Book newBook =
        Book.create(
            ...
            0);
    Book saved =
        persistBookWithAuthors(newBook, command.authors().stream().distinct().toList(), now);
  ...
  public Book updateBook(UpdateBookCommand command) {
    Book book =
        bookStore.findById(command.id()).orElseThrow(() -> new DomainException("Book not found"));
    Instant now = clock.instant();
    Book updatedBook =
        Book.create(
            book.id(),
            ...
            command.version());
    Book saved =
        persistBookWithAuthors(updatedBook, command.authors().stream().distinct().toList(), now);
    eventPublisher.publishEvent(new BookAuthorsChangedEvent(saved.id(), saved.authors()));
    return saved;
  }
  ...
  private Book enrichWithAuthors(Book book) {
    List<AuthorId> authors =
        bookAuthorLinkStore
            .findAuthorIdsByBookIds(List.of(book.id()))
