    ...
    return books.stream()
        .collect(
            Collectors.toMap(
                Function.identity(),
                book ->
                    authorsForBooks.getOrDefault(BookId.parse(book.id()), List.of()).stream()
                        .map(AuthorDto::map)
                        .toList()));
  }

  //    @SchemaMapping(typeName = "Book")
  //    public CompletableFuture<List<AuthorDto>> authors(BookDto bookDto, DataLoader<BookId,
  // BookAuthorsDataLoader.AuthorsWrapper> authorsDataLoader) {
  //        return
  // authorsDataLoader.load(BookId.parse(bookDto.id())).thenApply(BookAuthorsDataLoader.AuthorsWrapper::authors);
  //    }

  @QueryMapping
  public Page<BookDto> findBooks(ScrollSubrange subrange, @Argument List<SortInput> sort) {
    return pagination.page(
        subrange, sort, pageable -> bookService.getBooks(pageable), BookDto::map);
  }
}
