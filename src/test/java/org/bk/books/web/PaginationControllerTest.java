...
class PaginationControllerTest {
  ...
  void test_findAuthors_whenPageSizeChangesAfterEndCursor_returnsNextAuthorPage() {
    ...
    GraphQlTester.Response firstPage = graphQlTester.document(query).variable("first", 2).execute();

    String endCursor = firstPage.path("findAuthors.pageInfo.endCursor").entity(String.class).get();
    ...
    firstPage.path("findAuthors.pageInfo.hasPreviousPage").entity(Boolean.class).isEqualTo(false);

    GraphQlTester.Response secondPage =
        graphQlTester.document(query).variable("first", 3).variable("after", endCursor).execute();

    secondPage
        .path("findAuthors.edges[*].node.name")
        .entityList(String.class)
        .containsExactly("Douglas Adams", "Frank Herbert", "George Orwell");
    secondPage.path("findAuthors.pageInfo.hasNextPage").entity(Boolean.class).isEqualTo(true);
    secondPage.path("findAuthors.pageInfo.hasPreviousPage").entity(Boolean.class).isEqualTo(true);
  ...
  void test_findBooks_whenPageSizeChangesAfterEndCursor_returnsNextBookPage() {
    ...
    GraphQlTester.Response firstPage = graphQlTester.document(query).variable("first", 3).execute();

    String endCursor = firstPage.path("findBooks.pageInfo.endCursor").entity(String.class).get();
    ...
    firstPage.path("findBooks.pageInfo.hasPreviousPage").entity(Boolean.class).isEqualTo(false);

    GraphQlTester.Response secondPage =
        graphQlTester.document(query).variable("first", 2).variable("after", endCursor).execute();
    ...
    secondPage.path("findBooks.pageInfo.hasPreviousPage").entity(Boolean.class).isEqualTo(true);
